...
class ApiClientTest {
    ...
    void shouldCallApiClient() throws ApiException {
        ...
        Town test = townApi.create(new Town().name("test123").postCode("12345"));

        townApi.callList().stream().map(Town::getId).forEach(System.out::println);
        ...
        townApi.callList().stream().map(Town::getName).forEach(System.out::println);
